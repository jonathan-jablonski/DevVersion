...
const SubscribePost = () => {
	...
	const likePost = (id) => {
		axios.put(`http://localhost:3000/like`, { postId: id }, config)
			.then((result) => {
	...
	const UnlikePost = (id) => {
		axios.put(`http://localhost:3000/Unlike`, { postId: id }, config)
			.then((res) => {
				const newData = data.map((item) => {
	...
	const makeComment = (text, postId) => {
		setComment("");
		axios.put(`http://localhost:3000/comment`, { text, postId }, config)
			.then((result) => {
				const newData = data.map((item) => {
					if (result.data._id === item._id) return result.data;
					else return item;
	...
	const deletePost = (postId) => {
		axios.delete(`http://localhost:3000/deletepost/${postId}`, config).then((res) => {
