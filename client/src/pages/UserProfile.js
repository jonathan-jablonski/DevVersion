...
const UserProfilePage = () => {
	...
	useEffect(() => {
		axios.get(`http://localhost:3000/user/${userid}`, config).then((res) => {
			setData(res.data);
		});
	}, []);

	const followUser = () => {
		axios.put(`http://localhost:3000/follow`, { followId: userid }, config).then((result) => {
			dispatch({
				type: "UPDATE",
	...
	const UnfollowUser = () => {
		axios.put(`http://localhost:3000/unfollow`, { unfollowId: userid }, config).then((result) => {
			dispatch({
				type: "UPDATE",
				payload: { Followers: result.data.Followers, Following: result.data.Following },
			});
